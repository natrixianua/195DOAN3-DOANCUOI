...
class MailController extends Controller
{
    ...
    public function quen_mat_khau(Request $request){
        ...
        // return view('pages.home',compact('ao','quan','dongho','category')); //1
               return view('pages.checkout.forget_pass')->with('category',$cate_product)->with('brand',$brand_product)->with('all_product',$all_product)->with('showproduct',$show_product)->with('meta_desc',$meta_desc)->with('meta_keywords',$meta_keywords)->with('meta_title',$meta_title)->with('url_canonical',$url_canonical)->with('slider',$slider)->with('quan',$quan)->with('dongho',$dongho)->with('ao',$ao); //1 //1
           }
        ...
        // return view('pages.home',compact('ao','quan','dongho','category')); //1
               return view('pages.checkout.new_pass')->with('category',$cate_product)->with('brand',$brand_product)->with('all_product',$all_product)->with('showproduct',$show_product)->with('meta_desc',$meta_desc)->with('meta_keywords',$meta_keywords)->with('meta_title',$meta_title)->with('url_canonical',$url_canonical)->with('slider',$slider)->with('quan',$quan)->with('dongho',$dongho)->with('ao',$ao); //1 //1
           }
           public function recover_pass(Request $request){
            $data = $request->all();
            $now = Carbon::now('Asia/Ho_Chi_Minh')->format('d-m-Y');
            $title_mail = "Lấy lại mật khẩu hoangphuc.com".' '.$now;
            ...
            if($customer){
                ...
                $link_reset_pass = url('/update-pass?email='.$to_email.'&token='.$token_random);
                ...
                //--send mail
                return redirect()->back()->with('message', 'Vui lòng vào email để reset password');
            }
        }
    }
}
