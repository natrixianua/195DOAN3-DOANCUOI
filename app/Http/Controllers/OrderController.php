...
class OrderController extends Controller
{
	...
	public function history(Request $request){
    	...
    	// return view('pages.history.history')->with(compact('gethis'));
        ...
        //--seo
				...
				$gethis = Order::where('customer_id',Session::get('customer_id'))->orderby('order_id','DESC')->paginate(10);
        // return view('pages.home',compact('ao','quan','dongho','category')); //1
               return view('pages.history.history')->with('category',$cate_product)->with('brand',$brand_product)->with('all_product',$all_product)->with('showproduct',$show_product)->with('meta_desc',$meta_desc)->with('meta_keywords',$meta_keywords)->with('meta_title',$meta_title)->with('url_canonical',$url_canonical)->with('slider',$slider)->with('quan',$quan)->with('dongho',$dongho)->with('ao',$ao)->with('gethis',$gethis); //1 //1
           }
       }
       public function view_history_order(Request $request,$order_code){
        ...
        //--seo
				...
				// xem chi tiết lich sử
       			$order_details = OrderDetails::with('product')->where('order_code',$order_code)->get();
       			$gethis = Order::where('order_code',$order_code)->first();
       			
       			$customer_id = $gethis->customer_id;
       			$shipping_id = $gethis->shipping_id;
       			$order_status = $gethis->order_status;
               ...
               return view('pages.history.view_history_order')->with('category',$cate_product)->with('brand',$brand_product)->with('all_product',$all_product)->with('showproduct',$show_product)->with('meta_desc',$meta_desc)->with('meta_keywords',$meta_keywords)->with('meta_title',$meta_title)->with('url_canonical',$url_canonical)->with('slider',$slider)->with('quan',$quan)->with('dongho',$dongho)->with('ao',$ao)->with('order_details',$order_details)->with('customer',$customer)->with('shipping',$shipping)->with('coupon_condition',$coupon_condition)->with('coupon_number',$coupon_number)->with('gethis',$gethis)->with('order_status',$order_status); //1 //1
           }
       }
   }
